% Raised cosine filter an input signal, in order to obtain the output signal
...
function output = raised_cosine_filter(input, t, f_symbol)

% We need to work out the sample rate of the input signal 
f_s = (length(t)-1)/(t(end)-t(1));

% Design the raised cosine filter
D = fdesign.pulseshaping(round(f_s/f_symbol),'Raised Cosine','Nsym,Beta',10,0);
H = design(D);
B = tf(H);
