% Script that Manchester code modulates a quantised signal
...
[t_new,u] = func_manchester_encode(t, bits, bit_rate);

% Choose a noise power
N_0 = 0; % 1 should give errors

% Add some noise to our signal
u_received = u + sqrt(N_0)*randn(size(u));

% Perform Manchester decoding
recovered_bits = func_manchester_decode(t_new, u_received, bit_rate);

% Perform pulse coded demodulation
recovered_symbols = func_pulse_coded_demodulation(recovered_bits,k);

% Perform dequantisation
v_quantised = func_dequantise(recovered_symbols, quantisation_levels);
...
subplot(2,1,2,'XTick',0:1/bit_rate:max(t),'XGrid','on','YTick',[0 1],'YGrid','on','YLim',[-1,2],'box','on');
hold on
plot(t,u_received(1:length(t)));
xlabel('t [s]');
ylabel('u(t)');
