% Script that on-off keying modulates a quantised signal
...
% Perform OOK modulation
u2 = k_am*u.*cos(2*pi*f_c*t_new);
...
subplot(2,1,2,'XTick',0:1/bit_rate:max(t),'XGrid','on','YTick',[-k_am k_am],'YGrid','on','YLim',[-2*k_am,2*k_am],'box','on');
hold on
...
for bit_index=1:length(bits)
    text((bit_index-0.5)/bit_rate,1.25*k_am,num2str(bits(bit_index)), 'HorizontalAlignment', 'center','Clipping','on');
end
